...
- 日付: 2026-08-08（2026-08-08 改訂: task/final review指摘を受け、検索欄Escクリアの条件にヘルプパネルの開閉チェックを追加）
...
実装完了後、task reviewでまず、グループ管理モーダル・タグ管理モーダルが開いている間は検索欄Escクリアをスキップすべきという指摘を受けた（`groupManageOverlay.hidden && tagManageOverlay.hidden` を条件に追加）。いずれのモーダルもフォーカストラップを持たないため、モーダルを開いたままTabで背後の検索欄にフォーカスが移り、その状態でEscを押すと、モーダルを閉じずに検索をクリアしてしまう不具合があったため。

さらにその後、同一ブランチの最終レビューで同種の不具合がヘルプパネルにも存在することが判明した。`?` でヘルプパネルを開いたまま `/` で検索欄にフォーカスし、Escを押すと、ヘルプパネルを閉じずに検索をクリアしてしまう。ヘルプパネルもモーダル同様フォーカストラップを持たないため、同じ理屈で条件に `helpPanel.hidden` を追加する必要があった。この改訂で、検索欄Escクリアの条件は「グループ管理モーダル・タグ管理モーダル・ヘルプパネルのいずれも閉じている」の3条件すべてを満たす場合のみに揃えた。
...
変更後（2026-08-08改訂: task review・final reviewの指摘を反映し、グループ管理モーダル・タグ管理モーダル・ヘルプパネルがいずれも閉じている場合のみ検索クリアを行うよう条件を追加。詳細は本ファイル冒頭の「改訂の経緯」を参照）:
```js
    if (e.key === 'Escape') {
      if (e.target === searchInput && state.search && groupManageOverlay.hidden && tagManageOverlay.hidden && helpPanel.hidden) {
        clearSearch();
        return;
      }
      closeTopmostLayer();
...
`searchInput` はこの時点（ファイル内でこの `keydown` リスナーより手前）で既に宣言・代入済みの変数なので、そのまま参照できる。`groupManageOverlay`・`tagManageOverlay`・`helpPanel` も同様に手前で宣言済みのため、新規宣言は不要。
